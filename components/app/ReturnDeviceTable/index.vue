...
async function fetchData(offset: number, length: number, options: {
  ...
  const res = await receiptService.getReturnReceipts(offset, length, {
    searchText: options.searchText,
    searchFields: ['device_kind_id', 'device_kind_name', 'borrowed_place', 'returned_place'],
    sortField: options.sortField as any,
    desc: options.desc
  });
