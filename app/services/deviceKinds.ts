...
export const deviceKindService = {
  ...
  async getTotalItems(
    ...
    const { $cachedFetch } = useNuxtApp();
    return (
      await $cachedFetch('/api/device_kinds', {
        query: {
          category_id: categoryId,
          offset: 0,
          length: 1,
  ...
  async getById(
    ...
    const { $cachedFetch } = useNuxtApp();
    return await $cachedFetch(`/api/device_kinds/${deviceKindId}`, {
      ttl: 600,
      query: {
        lab_id: labId,
      },
  ...
  async getQuantityByLab(
    ...
    const { $cachedFetch } = useNuxtApp();
    return (
      await $cachedFetch('/api/device_kinds/quantity_by_lab', {
        query: {
          kind_id: deviceKindId,
          search_text: searchText,
          search_fields: searchFields,
        },
        ttl: 0,
