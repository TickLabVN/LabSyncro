...
<template>
  <div ref="dropdown" class="relative">
    <div class="relative">
      <Input v-model="searchText" class="bg-white border-2 w-full p-4 rounded-xl text-lg" type="search"
    ...
    <div v-if="searchItems.length"
      :class="`${isDropdownActive ? 'flex' : 'hidden'} flex-col gap-1 absolute bg-white p-1 mt-1 w-full z-50 shadow-[0_0px_16px_1px_rgba(0,0,0,0.3)]`">
      <button v-for="(item, index) in searchItems" :key="item.id"
